...
fn example(){
    ...
    let ksk = cpu_gen_ksk(&FHEParameters::PBS(pbs_params), &mut encryption_generator, &small_lwe_sk, &big_lwe_sk);
    let (bsk, fourier_bsk) = cpu_gen_bsk(&FHEParameters::PBS(pbs_params), &mut boxed_seeder, &small_lwe_sk, &glwe_sk);
    // let (multibsk, fourier_multibsk) = cpu_gen_multibsk(&FHEParameters::PBS(pbs_params), &mut encryption_generator, &small_lwe_sk, &glwe_sk);
    let pksk = cpu_gen_pksk(&FHEParameters::PBS(pbs_params), &small_lwe_sk, &glwe_sk, &mut encryption_generator);
    ...
    // apply gen pbs
    let clear1 = 17;
    let ct1 = cpu_encrypt(&&FHEParameters::PBS(pbs_params), &mut encryption_generator, &small_lwe_sk, clear1, true);
   
    let lut = &cpu_gen_lut(&FHEParameters::PBS(pbs_params), |x: u64| x as u64, true);
    let start = std::time::Instant::now();
    let ct2 = cpu_pbs(&fourier_bsk, &ct1, lut);
    println!("pbs took: {:?}", start.elapsed());    
    
    let f1 = |x: u64| (8) as u64;
    let f2 = |x: u64| x%2 as u64;
    let f3 = |x: u64| x - 1;
    let f4 = |x: u64| x + 1;

    let vec_functions = vec![f1, f2, f3, f4];
    let x: i32 = 0;
    let (mut lut, v) = cpu_gen_pbsmany_lut(&FHEParameters::PBS(pbs_params), vec_functions, x);
    let vec_pbs_bits = cpu_pbsmany(&fourier_bsk.as_view(), &ct1, &mut lut, v, x);

    let delta = 1 << 64 - 4 + x;
    for ct in vec_pbs_bits.iter(){
        let dec = cpu_decrypt_delta(&big_lwe_sk, &ct, delta);
        println!("PBSmany lut dec: {}", dec);
    }

    // cpu modlus wsiwtch 
    // let delta = 1 << 63;
    // for i in 0..vec_pbs_bits.len(){
    //     println!("i: {}", i);
    //     let mut ct = cpu_encrypt_custom(&FHEParameters::MultiBit(pbs_params), &mut encryption_generator, &small_lwe_sk, (i as u64) % 2, delta);
    //     let mut ct_copy = ct.clone();
    //     cpu_modswitch(&mut ct_copy, 62);
    //     lwe_ciphertext_add_assign(&mut ct, &ct_copy);



    //     // let ct_out = cpu_bootstrap_boolean_no_padding(&fourier_bsk, &mut ct);
    //     let dec = cpu_decrypt_delta(&small_lwe_sk, &ct, 1 << 61);
    //     println!("negacyclic bootstrap dec: {}", dec);
    // }
    ...
    // let cbs_bits_count = 8;
    // let mut vec_wopbs_bits: Vec<LweCiphertext<Vec<u64>>> = Vec::new();
    // let mut integer = 0;
    // for i in 0..cbs_bits_count{
    //     let clear = 1;
    //     integer += clear << (cbs_bits_count - i - 1);
    //     let mut ct = cpu_encrypt(&FHEParameters::Wopbs(wopbs_params), &mut wopbs_encryption_generator, &wopbs_small_lwe_sk, clear, false);
    //     let ct_out = cpu_bootstrap_boolean_no_padding(&wopbs_fourier_bsk, &mut ct);

    //     let start = std::time::Instant::now();
    //     let ct_temp = cpu_ksk(&ksk_wopbs_large_to_wopbs_small, &ct_out);
    //     println!("key switch took: {:?}", start.elapsed()); 

    //     vec_wopbs_bits.push(ct_temp);
    // }
    ...
    // //     let pbs_bit_switched = cpu_ksk(&ksk_pbs_large_to_wopbs_small, &pbs_bit);
    ...
    // for (index, bit_out) in vec_wopbs_bits.iter().enumerate(){
    //     let dec: u64 = cpu_decrypt(&FHEParameters::Wopbs(wopbs_params), &wopbs_small_lwe_sk, &bit_out, false);
    //     println!("dec: {}", dec);
    // }
...
fn bloom(){
    ...
    let ksk = cpu_gen_ksk(&FHEParameters::MultiBit(pbs_params), &mut encryption_generator, &small_lwe_sk, &big_lwe_sk);
    ...
    let pksk = cpu_gen_pksk(&FHEParameters::MultiBit(pbs_params), &small_lwe_sk, &glwe_sk, &mut encryption_generator);
